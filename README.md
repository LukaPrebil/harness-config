...
Two more packages complete the stack: [`pi-mcp-adapter`](https://pi.dev/packages/pi-mcp-adapter), pinned to `2.32.1` because `2.33.0` swaps the MCP SDK registry dependencies for `pkg.pr.new` preview tarballs, loads Notion, Slack, and Playwright from `pi/mcp.json`, plus each project's `.mcp.json` (host-specific config discovery stays off); [`pi-intercom`](https://pi.dev/packages/pi-intercom) tracks `0.13.x` and lets sessions message each other directly and lets delegated children escalate to their supervisor.
...
`pi-subagents` 0.67.0 drops extension-provided tools from an agent's `tools` allowlist ([#2134](https://github.com/nicobailon/pi-subagents/issues/2134)). The builtin `researcher` loses its four [pi-web-access](https://pi.dev/packages/pi-web-access) tools: `web_search`, `fetch_content`, `get_search_content`, and `source_check`. [PR #2143](https://github.com/nicobailon/pi-subagents/pull/2143) fixes it on `main`, but no release contains the fix. A verified workaround overrides `researcher` with `tools: "inherit"`, `excludeTools` for shell and edit tools, and `subagentOnlyExtensions: npm:pi-web-access`. That override works, but `tools: "inherit"` also exposes ambient MCP tools to the researcher, so it stays unapplied until that trade-off is accepted. Do not substitute an `extensions` allowlist: restricting extensions to `npm:pi-web-access` disables the `pi-ollama-cloud` provider extension, and the child then fails with `Model "ollama-cloud/deepseek-v4.1-flash:medium" not found`.
